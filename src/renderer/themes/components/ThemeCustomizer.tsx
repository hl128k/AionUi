...
import { Button, Card, ColorPicker, Form, Input, Message, Modal, Select, Space, Typography, Upload } from '@/renderer/components/themed-components';
import { IconDownload, IconUpload } from '@icon-park/react';
import React, { useCallback, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { useThemeSwitcher } from '../hooks';
import type { AppTheme, CodeHighlightTheme } from '../types';
import { convertExternalToAppTheme, generateSampleThemeYaml, parseYamlTheme, stringifyYamlTheme, validateThemeYaml } from '../yaml-utils';
...
export const ThemeCustomizer: React.FC<ThemeCustomizerProps> = ({ visible = false, onClose, editingTheme = null }) => {
    ...
    (file: File) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const validation = validateThemeYaml(content);
          ...
          const appTheme = convertExternalToAppTheme(themeData);
  ...
  const handleLoadSample = useCallback(() => {
    try {
      const sampleYaml = generateSampleThemeYaml();
        ...
        <Card title='主题操作'>
          <Space>
            <Upload accept='.yaml,.yml,.json' beforeUpload={handleImportTheme} showUploadList={false}>
              <Button icon={<IconUpload />}>{t('settings.importTheme')}</Button>
            </Upload>

            <Button icon={<IconDownload />} onClick={handleExportTheme}>
