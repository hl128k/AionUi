...
import { IconDelete, IconDownload, IconEye, IconMoonFill, IconPlus, IconRefresh, IconSettings, IconSunFill, IconUpload } from '@arco-design/web-react/icon';
      ...
      <Card title='主题管理'>
        <Space wrap>
          <Upload accept='.json' beforeUpload={handleFileUpload} showUploadList={false}>
            <Button type='primary' icon={<IconUpload />} loading={isImporting}>
              导入主题
            </Button>
          </Upload>

          <Button icon={<IconRefresh />} onClick={refreshThemes}>
            刷新列表
          </Button>

          <Button icon={<IconPlus />} type='outline'>
            创建主题
          </Button>
        </Space>
      </Card>
        ...
        <List
          ...
          render={(theme: AppTheme) => (
            <List.Item
              ...
              actions={[
                <Button key='preview' type='text' icon={<IconEye />} onClick={() => handlePreview(theme)}>
                  预览
                </Button>,
                <Button key='export' type='text' icon={<IconDownload />} loading={isExporting} onClick={() => handleExport(theme)}>
                  导出
                </Button>,
              ]}
            >
              <List.Item.Meta
                avatar={
                  ...
                  >
                    {theme.mode === 'light' ? <IconSunFill style={{ color: theme.codeHighlight.color }} /> : <IconMoonFill style={{ color: theme.codeHighlight.color }} />}
                  </div>
                }
                title={
                  <Space>
